      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-3 border-b border-[var(--border-primary)] bg-[var(--gold-primary)]/5 relative z-20">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[12px] font-mono font-bold tracking-[0.2em] text-[var(--gold-primary)]">[ BOTAZEZ // ENTITY INTEL ]</span>
                ...
                <div className="grid grid-cols-2 gap-x-6 gap-y-2 mt-2">
                  {Object.entries(selectedNode.properties).map(([k, v], i) => (
                    <div key={`${selectedNode.id}-${k}`}>
                      <span className="text-[12px] font-mono text-[var(--gold-primary)]/70 uppercase tracking-widest">{k.replace(/_/g, ' ')}</span>
        ...
        <div className="px-4 py-2 border-t border-white/5 flex items-center gap-3 flex-wrap">
          {Object.entries(TYPE_COLORS).map(([t, c]) => (
            <div key={t} className="flex items-center gap-1">
              ...
              <span className="text-[11px] font-mono text-white/40 uppercase">{t}</span>
            </div>
          ))}
        </div>
      </motion.div>
